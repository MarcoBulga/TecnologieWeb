...
-- * Generation date: Fri Jan 16 12:27:06 2026 
-- * LUN file: C:\Users\user\OneDrive\Desktop\uni\tecnologie web\db tecnologie web.lun 
-- * Schema: tecnologieWeb versione con tendine logico/SQL 
-- ********************************************* 
...
alter table possiede add constraint REF_possi_FILTR
     foreign key (nome)
     references FILTRO(nome)
     on delete cascade;

alter table possiede add constraint REF_possi_GRUPP_FK
     foreign key (idGruppo)
     references GRUPPO(idGruppo)
     on delete cascade;
...
create unique index ID_FILTRO_IND
     on FILTRO (nome);
...
create index REF_possi_GRUPP_IND
